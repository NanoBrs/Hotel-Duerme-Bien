...
class GestionReservas(tk.Frame):
    def __init__(self, parent, controlador):
        ...
        self.nreserva= 1 #Se crea contador de N de reserva
    ...
    def contador_reserva_actual(self,operacion): #Se crea para contar correctamente el N° de reserva
        if operacion == "mostrar ventana":
            aux_n = str(self.nreserva) #
            text_reserva=("Reserva de Habitación N°"+aux_n)
            return text_reserva
        if operacion=="incrementar":
            self.nreserva += 1
            nreserva= self.contador_reserva_actual("mostrar ventana")
    ...
    def generar_ventana(self, parent):
        ...
        text_reserva=self.contador_reserva_actual("mostrar ventana")
        ttk.Label(parent, text=text_reserva).place(x=5, y=10)
    ...
    def seleccionar_habitacion_extra(self,personas_restantes):
                
        cancelar = messagebox.askyesno(f"La habitación Seleccionada no es suficiente", "Quedan personas sin acomodar. ¿Desea seleccionar otra habitación?")
        if cancelar == True:
            ventana_habitaciones= tk.Toplevel(self.root)
            ventana_habitaciones.geometry("400x400")
            ventana_habitaciones.title("Selección de Habitaciones")

            habitacion_1 = tk.Label(ventana_habitaciones, text="Habitacion 1 seleccionada")
            id_habitacion_entry_1 = tk.Entry(ventana_habitaciones, state='readonly')

            habitacion_1.place(x=20, y=80)
            id_habitacion_entry_1.place(x=165, y=80, width=150)

            

            # Tabla para mostrar las habitaciones
            habitaciones_table = ttk.Treeview(
                ventana_habitaciones,
                columns=('ID', 'Numero', 'Precio', 'Camas', 'Piso', 'Capacidad', 'Tipo', 'Orientacion', 'Estado'),
                show='headings',  # Para mostrar solo las cabeceras de las columnas
                height=5  # Número de filas visibles, ajusta según tus necesidades
            )
            habitaciones_table.heading('ID', text='ID')
            habitaciones_table.heading('Numero', text='Numero de Habitacion')
            habitaciones_table.heading('Precio', text='Precio por Noche')
            habitaciones_table.heading('Camas', text='Camas')
            habitaciones_table.heading('Piso', text='Piso')
            habitaciones_table.heading('Capacidad', text='Capacidad')
            habitaciones_table.heading('Tipo', text='Tipo de Habitacion')
            habitaciones_table.heading('Orientacion', text='Orientacion')
            habitaciones_table.heading('Estado', text='Estado')
            
            # Ajustar anchos de las columnas
            habitaciones_table.column('ID', width=30)
            habitaciones_table.column('Numero', width=135)
            habitaciones_table.column('Precio', width=100)
            habitaciones_table.column('Camas', width=50)
            habitaciones_table.column('Piso', width=50)
            habitaciones_table.column('Capacidad', width=80)
            habitaciones_table.column('Tipo', width=105)
            habitaciones_table.column('Orientacion', width=72)
            habitaciones_table.column('Estado', width=80)
            
            # Colocar la tabla en la ventana secundaria
            habitaciones_table.pack(padx=20, pady=100, fill=tk.BOTH, expand=True)

            # Combobox para seleccionar tipo de habitación
            label_tipo_habitacion = tk.Label(ventana_habitaciones, text="Tipo de Habitacion:")
            label_tipo_habitacion.place(x=20, y=20)

            #Cargar tipos de habitacion
            combo_tipo_habitacion = ttk.Combobox(ventana_habitaciones)
            combo_tipo_habitacion.place(x=150, y=20, width=200)
            rows = self.db.cargar_tipos_habitacion()
            tipos = {row['id_tipo_habitacion']: f"{row['tipo']} - Maximo {row['cantidad_maxima']} personas" for row in rows}
            self.tipo_habitacion_dict = tipos
            combo_tipo_habitacion['values'] = list(tipos.values())


            habitaciones_table.bind("<Double-1>", lambda event: self.cargar_datos_seleccionados(event))

        
        
            # Botones "Seleccionar" y "Cancelar"
            btn_seleccionar = tk.Button(ventana_habitaciones, text="Seleccionar", command=lambda: self.cargar_datos_seleccionados(habitaciones_table))
            btn_seleccionar.pack(side=tk.LEFT, padx=10, pady=10)


            # Boton Buscar Habitaciones
            tk.Button(ventana_habitaciones, text="Buscar Habitaciones", command=self.buscar_habitaciones).place(x=60, y=60, width=150)

            btn_cancelar = tk.Button(ventana_habitaciones, text="Cancelar", command=ventana_habitaciones.destroy)
    ...
    def registrar_reserva(self):
        try:
            ...
            contador_n_hab = str(self.nreserva)
            
            #print(f"Trabajando Habitación:{self.nreserva}")
            ...
            habitaciones_id = [] # Lista para guardar id de las habitaciones en la reserva actual
            capacidades = [] #lista donde se dumara la capacidades solicitadas en total sumando habitaciones
            personas_restantes = total_personas


            while personas_restantes > 0:
                habitacion_seleccionada,capacidad_habitacion_seleccionada = self.seleccionar_habitacion_extra(personas_restantes)
                if not habitacion_seleccionada:
                    messagebox.showerror("Error", "No hay habitaciones disponibles para acomodar a todas las personas.")
                    return
                habitaciones_id.append(habitacion_seleccionada)
                capacidad_habitacion = habitacion_seleccionada['capacidad']
                capacidades.append(capacidad_habitacion)
                personas_restantes -= capacidad_habitacion
